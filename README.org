...
*** Layer 0: Tailscale Mesh Network
CLBSD provides a small layer of abstraction to easily setup a new Tailscale mesh network for all your personal devices. 
...
**** Data Abstraction
Every container has an isolated ~ZFS~ dataset that lies on a ~zpool~. This allows for clear separation and sharing of data, with redundnacy and extremely easy migrations. ZFS datasets are logical partitions that can grow dynamically and package together filesystem, data, and snapshot history.
...
Docker containers are created as minimally as possible, running an Alpine microvm that supports Docker.
